...
규칙:
1. 병음 형식은 두 가지를 모두 지원합니다:
   - 괄호 있음: "한자 [병음] 한국어 뜻" (예: 打扫 [dǎsǎo] 청소하다)
   - 괄호 없음: "한자 pinyin 한국어 뜻" (예: 哪儿 nǎr 어디)
   두 형식 모두 pinyin 필드에 성조 포함 병음을 추출하세요
2. 한자 앞의 * 기호는 제거하세요
3. 한국어 뜻은 병음 뒤의 한글 텍스트 전체입니다
4. "한자 [병음] / 한자 [병음] 한국어 뜻" 형식은 두 한자가 같은 뜻을 공유하는 것입니다. 각 한자를 같은 한국어 뜻으로 별도 entry로 분리하세요
   예시: "矮 [ǎi] / 低 [dī] 작다" → {"chinese": "矮", "pinyin": "ǎi", "korean": "작다"}, {"chinese": "低", "pinyin": "dī", "korean": "작다"}
5. 반의어, 상위어, 하위어 등 섹션 제목(한국어만으로 된 텍스트)은 추출하지 마세요
6. 여러 섹션이나 열에 걸쳐 배치된 단어도 빠짐없이 모두 추출하세요
7. 이미지에 중국어가 없으면 {"words": []}
...
유형 3 (type3): 어구풀이 페이지 - 번호+굵은 중국어 표현문장이 섹션 헤더이고, 불릿(•) 예문들이 중국어+한국어 쌍으로 나열됨
유형 4 (type4): 교과서 본문 대화 페이지 - 한국어 번역 없이 중국어 대화문만 나열된 구조 (본문해석 섹션이 없음)

반드시 다음 JSON 형식으로만 응답하세요:
{"type": "type1"} 또는 {"type": "type2"} 또는 {"type": "type3"} 또는 {"type": "type4"}
...
WORKBOOK_TYPE4_PROMPT = """이미지는 중국어 교과서의 본문 대화 페이지입니다. 한국어 번역 없이 중국어 대화문만 있습니다.
각 대화 줄의 중국어 본문을 순서대로 추출하고, 각 줄의 한국어 번역을 생성하여 다음 JSON 형식으로만 응답하세요:
{"entries": [{"chinese_text": "중국어 본문(한자만)", "korean": "한국어 번역"}, ...]}

규칙:
1. 대화 줄마다 하나의 entry입니다
2. chinese_text는 한자로 된 중국어 본문만 추출합니다 (병음, 화자 레이블 제외)
3. korean은 해당 중국어 줄의 자연스러운 한국어 번역을 AI가 직접 생성합니다
4. 대화 순서를 유지합니다
5. JSON 외 다른 텍스트는 절대 포함하지 마세요"""

WORKBOOK_TYPE3_PROMPT = """이미지는 중국어 교재의 어구풀이 페이지입니다.
번호와 굵은 중국어 표현문장(헤더)이 있고, 그 아래 불릿(•) 예문들이 중국어+한국어 쌍으로 나열되어 있습니다.
불릿 예문만 추출하여 다음 JSON 형식으로만 응답하세요:
{"entries": [{"chinese_text": "예문 중국어(한자만)", "korean": "한국어 해석"}, ...]}

규칙:
1. 번호+굵은 헤더 문장은 제외합니다
2. 불릿(•) 예문만 추출합니다: chinese_text는 중국어 원문(한자만, 병음 제외), korean은 한국어 해석
3. 섹션 순서와 예문 순서를 유지합니다
4. 어휘 섹션은 제외합니다
5. JSON 외 다른 텍스트는 절대 포함하지 마세요"""
...
def _parse_type_response(text: str) -> str:
    ...
    if json_match:
        ...
        if detected in ("type1", "type2", "type3", "type4"):
            return detected
    return "type1"
...
async def extract_workbook(file_path: str, workbook_type: str) -> list[dict]:
    if workbook_type == "type1":
        prompt = WORKBOOK_TYPE1_PROMPT
    elif workbook_type == "type3":
        prompt = WORKBOOK_TYPE3_PROMPT
    elif workbook_type == "type4":
        prompt = WORKBOOK_TYPE4_PROMPT
    else:
        prompt = WORKBOOK_TYPE2_PROMPT
    provider = os.getenv("AI_PROVIDER", "anthropic").lower()
    if provider == "openai":
...
유형 1 (type1): 표(테이블) 형식으로 한자, 의미, 예문 등이 행으로 나열된 페이지
유형 2 (type2): 대화문이 있는 유형학습 페이지 (A:, B: 등 화자별 대화 + 본문해석 섹션이 별도로 있음)
유형 3 (type3): 어구풀이 페이지 - 번호+굵은 중국어 표현문장이 헤더이고, 불릿(•) 예문들이 중국어+한국어 쌍으로 나열됨
유형 4 (type4): 교과서 본문 대화 페이지 - 한국어 번역 없이 중국어 대화문만 나열된 구조 (본문해석 섹션 없음)
...
- 한자 앞의 * 기호 제거
- **chinese 필드에는 반드시 중국어 한자만 입력. 한국어/한글이 절대 들어가면 안 됨** (예: "从", "到", "离" 등 중국어 문자만)
- **병음(pinyin)이 표에 명시되어 있지 않으면, 해당 한자의 정확한 병음을 직접 생성하세요** (예: 从→cóng, 离→lí, 当→dāng)
- meaning은 표에 있는 한국어 뜻/용법을 추출 (예: "~(으)로부터", "~에게", "~보다" 등)
- 예문(example)은 중국어 원문만 (한국어 해석 제외)
...
유형 3인 경우 다음 JSON으로 응답:
{"type": "type3", "entries": [{"chinese_text": "예문 중국어(한자만)", "korean": "한국어 해석"}, ...]}

유형 3 규칙:
- 번호+굵은 헤더 문장은 제외
- 불릿(•) 예문만 추출: chinese_text는 한자 원문만(병음 제외), korean은 한국어 해석
- 섹션 순서와 예문 순서 유지
- 어휘 섹션 제외

유형 4인 경우 다음 JSON으로 응답:
{"type": "type4", "entries": [{"chinese_text": "중국어 본문(한자만)", "korean": "한국어 번역"}, ...]}

유형 4 규칙:
- 대화 줄마다 하나의 entry
- chinese_text는 한자 본문만 (병음, 화자 레이블 제외)
- korean은 해당 줄의 자연스러운 한국어 번역을 AI가 직접 생성
- 대화 순서 유지
...
def _parse_combined_response(text: str) -> dict:
    ...
    if json_match:
        ...
        if wb_type not in ("type1", "type2", "type3", "type4"):
            wb_type = "type1"
