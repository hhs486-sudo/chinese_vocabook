...
7. **이미지에 실제로 보이는 텍스트만 추출하세요. 이미지에 없는 내용을 추측하거나 생성하지 마세요.**
8. 병음이 이미지에 없으면 빈 문자열로 처리하세요. 절대 직접 생성하지 마세요."""
...
2. 한자 앞의 * 기호는 제거하세요
3. 예문(example)은 이미지에 실제로 보이는 중국어 예문만 추출하세요. 예문이 이미지에 있는데 누락하지 마세요.
...
**핵심 원칙: 이미지에 실제로 보이는 텍스트만 추출하세요. 이미지에 없는 내용을 절대 추측하거나 생성하지 마세요.**
...
- 한자 앞의 * 기호 제거
- chinese 필드에는 이미지에 보이는 중국어 한자만 입력. 한국어/한글이 절대 들어가면 안 됨
- 병음(pinyin)은 이미지에 실제로 표시된 것만 추출하세요. 이미지에 없으면 빈 문자열("")로 처리하세요.
- meaning은 표에 있는 한국어 뜻/용법을 이미지에서 그대로 추출
- 예문(example)은 이미지에 실제로 있는 중국어 원문만 추출 (한국어 해석 제외)
- 예문이 이미지에 없는 행만 빈 문자열 처리
- 의미가 여러 개면 쉼표 구분
...
- chinese_text는 이미지에 보이는 한자 본문만 (병음 제외)
- korean은 이미지의 본문해석에서 해당 화자의 한국어 번역 그대로 추출
- 어휘 섹션 제외
