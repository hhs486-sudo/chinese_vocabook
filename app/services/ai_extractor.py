...
5. "한자 [병음] / 한자 [병음] 한국어 뜻" 형식은 두 한자가 같은 뜻을 공유하는 것입니다. 각 한자를 같은 한국어 뜻으로 별도 entry로 분리하세요
   예시: "矮 [ǎi] / 低 [dī] 작다" → {"chinese": "矮", "pinyin": "ǎi", "korean": "작다"}, {"chinese": "低", "pinyin": "dī", "korean": "작다"}
6. 반의어, 상위어, 하위어 등 섹션 제목(한국어만으로 된 텍스트)은 추출하지 마세요
7. 여러 섹션이나 열에 걸쳐 배치된 단어도 빠짐없이 모두 추출하세요
8. 이미지에 중국어가 없으면 {"words": []}
9. JSON 외 다른 텍스트는 절대 포함하지 마세요"""
...
{"type": "type1"} 또는 {"type": "type2"} 또는 {"type": "type3"}
...
def _parse_type_response(text: str) -> str:
    ...
    if json_match:
        ...
        if detected in ("type1", "type2", "type3"):
            return detected
    return "type1"
...
def _parse_combined_response(text: str) -> dict:
    ...
    if json_match:
        ...
        if wb_type not in ("type1", "type2", "type3"):
            wb_type = "type1"
